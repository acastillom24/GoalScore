...
class DataProcessor:
    ...
    def load_matches(path: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Teams found: {sorted(df['home_team'].unique())}")
            return df
...
class SimplifiedPoissonModel:
    ...
    def fit(self, matches: pd.DataFrame):
        ...
        try:
            ...
            self.is_fitted = True
            logger.info(f"✓ Simplified Poisson model fitted successfully")
            logger.info(
                f"  League averages: Home {self.league_avg_home:.2f}, Away {self.league_avg_away:.2f}"
            )
    ...
    def predict_match(self, home_team: str, away_team: str, max_goals: int = 8) -> Dict:
        ...
        home_stats = self.team_stats.get(
            home_team,
            {"attack_home": self.league_avg_home, "defense_home": self.league_avg_away},
        )
        away_stats = self.team_stats.get(
            away_team,
            {"attack_away": self.league_avg_away, "defense_away": self.league_avg_home},
        )
...
class FootballPredictor:
    ...
    def train_all_models(self):
        ...
        logger.info(f"   ✓ {len(self.elo_model.ratings)} teams rated")
